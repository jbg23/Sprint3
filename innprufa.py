...
breidd = 800
haed = 600
startImage = pygame.image.load('game-of-thrones-dragonstone-cr-hbo.jpg')
bakgrunnurGameIntro = pygame.image.load('3f389901818aa313068e68e70aea59b8.jpg')
level=0
...
#pyGame picture
def picture(hogwartsImage):
    hogwartsImage = pygame.transform.scale(hogwartsImage,size)
    gameDisplay.blit(hogwartsImage,(0,0))

#Game start level board
def leikurIntro():
    picture(startImage)
    draw_stick_figure(screen)
    messageDisplayLevel('Game Of Thrones. Valdatafl',2)
    takkar("Hefja Leik",337,450,150,75,GRAY,GREEN,'StartLevel1')
#Level 1 board
def level1Intro():
    picture(bakgrunnurGameIntro)
    draw_stick_figure(screen)
    messageDisplayLevel('Velkomin/nn í valdatafl leikinn!',7)
    messageDisplayLevel('Við erum staðsett leikkarakter borðinu', 4.5)
    messageDisplayLevel('Velja persónu?', 3)
    takkar("Já!",150,450,150,75,GRAY,GREEN,'StartLevel1B')
...
def main():
    ...
    while not done:
        ...
        if level ==0:
            if state_tune !=0:
                music('GameofThronesTheme.mp3')
                state_tune=0
            leikurIntro()
        elif level ==1:
            if state_tune == 0:
                music('Damiano_Baldoni_-_A_Ghr.mp3')
                state_tune =1
            level1Intro()
